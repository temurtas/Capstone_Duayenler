...
void loop()
{
  ...
  p_time = t;
  // ********************
  Kp = 0.133; // Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19, gün sonu=18 , uzunvid=19,205, bidön=18 , cp1=145, 
  //cp2=136
  Kd = 0.12825 ; // Feedback gains (delta) , ilk çalışan=0.11 , hizli=0.12 , gün sonu=13 , uzunvid=158,165, bidön=13 , cp1=1325,124,126, 
  //cp2=1255
  Ki = 0.025 ; // Feedback gains (sum) , ilk çalışan=0.06 , hizli=0.075 ,gün sonu=06 , uzunvid= 058,615, bidön=06 , cp1=02, 
  //cp2025
  // ********************
  ...
  if (stop_state == 1) {
    ...
    analogWrite(left_motor_pwm, 0 ) ;
      ...
      // ********************
      if (error >= 0) {
        pwm_offset = max ( 48 - 1.275 * beta , Kmin1) ; // 29,25 , hizli=36,0.7,25,26 gün sonu=34,.7,26, uzunvid=54,1.875,26-56,1.875,26 ,bidön=50,1.6,26
        // cp1=44,1.2,26, cp2=44,1.2,26
        ...
        pwm_offset = max ( 51 - 1.128 * beta , Kmin2) ; // 29,25 , hizli=36,0.7,25,26 , gün sonu=39,.7,29, uzunvid=60,1.78,30-61,1.76,30, bidön=54,1.575,30
        // cp1=47,1.15,29, cp2=51,1.128,31,33
