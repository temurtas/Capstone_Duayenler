...
// DUAYENLER Ltd. Şti
// 10.4 Volts is crutial for the regulator output
// 3.94
...
void loop()
{
  ...
  p_time = t;
  // ********************
  Kp = 0.192; // 191// Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19, gün sonu=18 , uzunvid=19,205, bidön=18 , cp1=145, cp3=1453
  Kd = 0.1545 ; /// 15475/ Feedback gains (delta) , ilk çalışan=0.11 , hizli=0.12 , gün sonu=13 , uzunvid=158,165, bidön=13 , cp1=1325,124,126, cp3=1345
  Ki = 0.7 ; // Feedback gains (sum) , ilk çalışan=0.06 , hizli=0.075 ,gün sonu=06 , uzunvid= 058,615, bidön=06 , cp1=02, cp3=028
  // ********************
  ...
  if (stop_state == 1) {
    ...
    analogWrite(left_motor_pwm, 0 ) ;
      ...
      if(yon == 0){
        pwm_offset = max ( 53 - 1.81 * beta , Kmin1) ; // 29,25 , hizli=36,0.7,25,26 gün sonu=34,.7,26, uzunvid=54,1.875,26-56,1.875,26 ,bidön=50,1.6,26
        // cp1=44,1.2,26, cp3=44,1.2,26,28
        //if (abs(error) > 70){pwm_offset=26;}
        }
        else{
        pwm_offset = max ( 62 - 1.8 * beta , Kmin2) ; // 29,25 , hizli=36,0.7,25,26 , gün sonu=39,.7,29, uzunvid=60,1.78,30-61,1.76,30, bidön=54,1.575,30
