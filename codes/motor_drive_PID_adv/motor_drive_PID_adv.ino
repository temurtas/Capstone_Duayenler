...
void loop()
{
  ...
  p_time = t;
  // ********************
  Kp = 0.19; // Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19
  Kd = 0.12 ; // Feedback gains (delta) , ilk çalışan=0.11 , hizli=0.12
  Ki = 0.075 ; // Feedback gains (sum) , ilk çalışan=0.06 , hizli=0.075
  // ********************
  maxSum = 4; // Optional integral term,
  ...
  if (error >= 0) {
    ...
    if (stop_signal == 1){
      ...
      pwm_offset = max ( 38 - 0.7*beta , 26) ; // 29,25 , hizli=36,0.7,25,26
    ...
    if (stop_signal == 1){
      ...
      pwm_offset = max ( 40 - 0.7*beta , 28) ; // 29,25 , hizli=36,0.7,25,26
      pwm_delta  = min( pwm_offset  + motorCmd, dacMax);
    }
    analogWrite(right_motor_pwm, pwm_offset );
    analogWrite(left_motor_pwm, pwm_delta ) ; // Left PWM is Pin 5 
    initial_start();
