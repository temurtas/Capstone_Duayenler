...
bool RUN = false; //SET THIS TO FALSE TO WORK WITH HANDSHAKE
...
int low_H = 49, low_S = 48, low_V = 130; // low_H = was 49 , High_S= was 100
...
{
	    cv::Mat dilating;
	cv::Mat element = getStructuringElement(cv::MORPH_RECT, cv::Size(21,21), cv::Point(10,10));
	dilate(src_gray, dilating, element);
	double largest_area = 0;
    cv::Mat canny_output;
    Canny( dilating, canny_output, 133, 400, 5, true);
    //canny_output = src_gry;			
    std::vector<std::vector<cv::Point> > contours;
    findContours( dilating, contours, cv::RETR_CCOMP, cv::CHAIN_APPROX_SIMPLE );
    ...
    for( size_t i = contours.size()-1; i< contours.size(); i++ )
    {
        //cv::Scalar color = cv::Scalar(255);
        cv::drawContours( drawing, hull, (int)i	, 255 , cv::FILLED);
    }

    //cv::namedWindow("Dilate demo");
    //imshow( "Dilate demo", dilating );
...
int main(int argc, char* argv[]) {
    ...
    double baseTime = (double) cv::getTickCount();
    while (true) {
        ...
        if(!RUN || (freading < 80)) {
            ...
            // dilate the ROI
            cv::Mat element = getStructuringElement(cv::MORPH_RECT, cv::Size(21,21), cv::Point(10,10));
            dilate(frame_threshed, frame_threshed, element);
            /**/
            std::vector<std::vector<cv::Point> > contours;	    
            findContours( frame_threshed, contours, cv::RETR_CCOMP, cv::CHAIN_APPROX_SIMPLE );    
            std::vector<std::vector<cv::Point> >hull( contours.size() );   

            for( size_t i = 0; i < contours.size(); i++ )
            {convexHull( contours[i], hull[i] ); }
            //std::cout << hull << std::endl;
            cv::Mat drawing = cv::Mat::zeros( frame_threshed.size(), CV_8UC1);
            for( size_t i = contours.size()-1; i< contours.size(); i++ )
            {cv::drawContours( drawing, hull, (int)i	, 255 , cv::FILLED);
            }
            lanedetector.setLineBorders(frame_threshed);
            const int max_thresh = 255;
            src_gray = frame_threshed;
            drawing = thresh_callback( 0, 0 );
            frame_threshed = drawing;
            /**/
            ...
            /**/
                        std::ofstream myFile;
                    myFile.open("testTime.txt", std::ios_base::app);
            myFile << ((double) cv::getTickCount() - baseTime)/ cv::getTickFrequency() * 1000 << " " << (avgRunTime / fps)  << "\n" ;
            /**/
                avgCounter = 0;
                avgRunTime = 0;
            } else
...
void server_funct() {
    ...
    while (1) {
        ...
        if (freading <= catchThreshold) {
            ...
            //turnOffThread = std::thread{turnOffRED};
            // TURN OFF RED HERE
            
            FD_ZERO(&rfd);
            FD_SET(new_socket, &rfd);
            timeout.tv_sec =  itout;
            timeout.tv_usec = utout;
            printf("Waiting...\n");
            ...
            if (ret != 0) {
                ...
                if (strcmp(buffer,ack) == 0) {
                    ...
                    //turnOffThread = std::thread{turnOffBLUE};
                    RUN = false;
                    break;
            ...
            if (ret != 0) {
                ...
                if (strcmp(buffer,ctch) == 0) {
                    ...
                    if (breading <= catchThreshold) {
                        ...
                        //turnOffThread = std::thread{turnOffGREEN};
                        FD_ZERO(&rfd);
                        FD_SET(new_socket, &rfd);
                        timeout.tv_sec = itout;
                        timeout.tv_usec = utout;
                        ...
                        // TURN OFF YELLOW HERE
                        //turnOffThread = std::thread{turnOffYELLOW};
                    }
                } else
                    memset (buffer, 0x00, 5);
            }
