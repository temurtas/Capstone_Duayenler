...
void LaneDetector::setLineBorders(cv::Mat img) {
    ...
    for (int i = 0; i < width; i++) {
        std::cout << whitePixels[i] << std::endl;
        }*/
...
//	findLineCenter(counter_x_left, sum_x_left, counter_x_right, sum_x_right);
    ...
    for (auto i : lines) {
        ini = cv::Point(i[0], i[1]);
        fini = cv::Point(i[2], i[3]);
        if ((std::abs(ini.y - fini.y) <= 3) || (std::abs(ini.x - fini.x) <= 2)) {
            continue;
        }
...
std::vector<cv::Point> LaneDetector::regression(
    ...
    output[0] = cv::Point(right_ini_x-25, ini_y);
    output[1] = cv::Point(right_fin_x-30, fini_y);
    output[2] = cv::Point(left_ini_x+30, ini_y);
    output[3] = cv::Point(left_fin_x+25, fini_y);
