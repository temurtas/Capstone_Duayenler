...
// Lipo Cell Voltages at the time of Tuning :: 3.93-3.91-3.89
...
{ // Stop State
  if (stop_state == 1) {
    ...
    p_time = t;
    // ********************
    Kp = 0.2225; // Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19, gün sonu=18 , uzunvid=19
    ...
    if (yonFin == 1 ){  // CCW Movement on the path
      // ********************
      Kp_ccw = 0.178; // Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19, gün sonu=18 , uzunvid=19
      Kd_ccw = 0.19 ; // Feedback gains (delta) , ilk çalışan=0.11 , hizli=0.12 , gün sonu=13 , uzunvid=158
      ...
      pwm_offset = max ( 29 - 1.475 * beta , Kmin_ccw) ;
      pwm_delta  = min( pwm_offset  + motorCmd, dacMax);
    } 
    else{  // CW Movement on the path
      // ********************
      Kp_cw = 0.295; // Feedback gains (proportional) , ilk çalışan=0.15,16 , hizli=19, gün sonu=18 , uzunvid=19
      Kd_cw = 0.18 ; // Feedback gains (delta) , ilk çalışan=0.11 , hizli=0.12 , gün sonu=13 , uzunvid=158
      ...
      pwm_offset=max ( 46 - 1.04* beta , Kmin_cw) ;;
      pwm_delta  = min( pwm_offset  + motorCmd, dacMax);
    }
    // ********************
    if (error > 0 ) {
