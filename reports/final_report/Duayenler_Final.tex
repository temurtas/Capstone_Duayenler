...
%$$\boxed{ G_p(s)=\frac{T(s)}{G_c(s)-T(s)G_c(s)} }$$ 
	...
	%	\DontPrintSemicolon	
	%// Update pastYon array \\ 
	%\For {int i = (yonSize - 1) \Rightarrow  0} {
	%	pastyon[i] = pastyon[i - 1]
	%}
	%$ pastyon[0]=yon $\\
	% yonSum $=$ 0 \\
	%\For {int k = 0 \Rightarrow (yonSize-1) }
	%{
	%	$ yonSum = yonSum + pastyon[k] $
	%}
	%$ yonAvg=yonSum/yonSize $\\ 
...
\subsubsection{Speed Subsystem}
	\todo{This part is rewritten for the final report}
	\begin{enumerate}
	\item {Requirements for the Solution}
...
\section{Budget}
	...
	\todo{Like 200 dolar hesap istiyor.}
