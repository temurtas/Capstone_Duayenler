...
\begin{document}
	...
	The subsystem is the first step of safely computing with an opponent in a racing path. This subsystem uses two time of flight distance sensor which is enhanced IR sensor. one at the back of the vehicle responsible for detecting the chasing opponent and one at the front of the vehicle responsible for detecting the chased opponent. The subsystem produces positive output if the chasing vehicle or chased vehicle is within a range of 5 cm from the vehicle.\\
...
for (lines_is_valid)	
	...
	Sample codes to implement the same communication in C++ and C are in Script \ref{sc:serial_c++} and Script \ref{sc:serial_c} respectively.
