...
\begin{document}
	...
	\textbf{Sensing System Requirements:}
	\begin{itemize}
	...
	\textbf{Lane Detection Subsystem Requirements:}	
	\begin{itemize}
	...
	The subsystem is the first step of safely computing with an opponent in a racing path. This subsystem uses two time of flight distance sensor which is enhanced IR sensor. one at the back of the vehicle responsible for detecting the chasing opponent and one at the front of the vehicle responsible for detecting the chased opponent. The subsystem produces positive output if the chasing vehicle or chased vehicle is within a range of 5 cm from the vehicle. Since the sensor reading is planned to be performed using Arduino, the required trigger for handshake protocol can be  easily send through serial commutation or directly implemented in the Arduino as an alternative.\\
	
	\textbf{Vehicle Detection Subsystem Requirements}
	...
	Computation system has two main subsystems which are Data Processing Subsystem and PID Controller Subsystem.\\
	
	\textbf{Computation System Requirements}
	\begin{itemize}
...
for (lines_is_valid)	
	...
	\todo{değiştir!!!!!}
	The output of the data processing subsystem does not mean anything for the vehicle. The motors should be driven using some sort of closed loop system. PID controllers are the most used controller in the robotics field. The purpose of the PID controller is basically to eliminate the error from the desired steady state. In our case the desired steady state  error to be companseted by the ...The requirements of this subsystem are listed below.\\
	
	
	\textbf{PID Controller Subsystem Requirements:}
	\begin{itemize}
	...
	\textbf{Communication System Requirements:}
	\begin{itemize}
	...
	\textbf{Internal Communication Subsystem Requirements:}
	\begin{itemize}
	...
	\begin{enumerate}
		...
		\item On Arduino side, \lstinline[language=Arduino]|Serial.begin(9600)| command should be executed and serial port should be read repeatedly to capture the incoming data
	...
	Python has a library called serial by which any type of data can be send through serial ports. Script \ref{sc:serial} is used to declare a serial object. Then using \lstinline[language=Python]|ar.write("some string \r".encode())|, the string "some string" can be send to Arduino. Note that "\r" carriage return character carries a high importance because it shows that a string is terminated and any other incoming data belongs to the new piece of string.  
	...
	As alternatives, the implementation on C is also examined. 
	
	Sample codes to implement the same communication in C is in Script \ref{sc:serial_c}. Since it is a low level language, specification of buffer size and other parameters should be done in the code.
...
RS232_OpenComport(cport_nr, bdrate, mode)
	...
	On Arduino  side, there are also several option that we can read the incoming data. Using \lstinline[language=Arduino]|Serial.read()| command is one of the simples solutions. However, it contains some issues like conversion from string to integer and when to stop. Furthermore, the incoming data should be considered in character basis for the exact control. \\
	
	There is also a more sophisticated solution. There is a \lstinline[language=Arduino]|<SerialCommand.h>| library for the Arduino which allows executing a function depending on the incoming string. Using \lstinline[language=Arduino]|.addCommand("str",func)| of the library any function can be associated with any string coming from serial port. Moreover, the functions can have argument. For example, let the string "PWMSET" be execute a function \lstinline[language=Arduino]|setpwm()| but the pwm value is required. If incoming string is of the form "PWMSET 150", using \lstinline[language=Arduino]|.next()| function of the library, the value 150 can be read and converted into integer and interpreted as the PWM value to be set. 
	...
	\textbf{External Communication Subsystem Requirements:}	
	\begin{itemize}
	...
	\textbf{Direction Subsystem Requirements:}
	\begin{itemize}
	...
	\textbf{Speed Subsystem Requirements:}
	\begin{itemize}
	...
	\textbf{Structure System Requirements:}
	\begin{itemize}
	...
	\textbf{Chasis Subsystem Requirements:}	
	\begin{itemize}
	...
	\textbf{PCB Subsystem Requirements:}
	\begin{itemize}
	...
	\textbf{Motion System Requirements:}
	\begin{itemize}
	...
	\textbf{Wheels Subsystem Requirements:}
	\begin{itemize}
	...
	\textbf{Motors Subsystem Requirements:}
	\begin{itemize}
