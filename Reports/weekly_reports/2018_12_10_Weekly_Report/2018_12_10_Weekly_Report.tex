...
\vspace*{-1.7cm}
\begin{center}
	\Large\textbf{December 03 - 09 Weekly Report}
	\end{center}
...
\section{Progress}
\begin{itemize}
	\item New motor and wheel pair was ordered.
	
	The process of determining the right motor was as follows; 
	\begin{itemize}
		\item $$ F=\frac{\tau}{R}$$ where F is the force the wheel applies to the ground.
		\item Weight Estimation $\approx1\ kg$, which includes the followings 
			\begin{itemize}
				\item Powerbank
				\item Motors
				\item Wheels
				...
				\item Chassis
			\end{itemize}
		which takes approximately 939 gr,
	 \item With $R=1.5\ cm$, our estimation was that $\tau=3\ kg-cm$ was enough for our purposes.
	 \item For that purpose \textit{Karbon 6V 600Rpm Dc Motor} at  was chosen, it has $\tau=\ 3,1\ kg-cm$ torque value at $6V$. Since we are planning the use this motor at $12V$, it is a fair assumption for our worst case scenario.
	 
	 
	  
	\end{itemize}
	
	\item Opencv was further studied. Our main algorithm for image processing includes as follows;
		\begin{itemize}
			\item Image Sharpening
			\item HSV Color Filtering
			\item Canny Edge Detection
			\item Hough Line Transformation
		\end{itemize}
	
	\item The critical parts were proposed
		\begin{itemize}
			\item Physical Module 
				\subitem *Line-follower based robot
			\item Software Module
				\subitem *Detection of Lane boundaries
		\end{itemize}
